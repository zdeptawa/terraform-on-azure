...
resource "azurerm_virtual_machine" "demo_db01" {
  ...
  storage_os_disk {
    name              = "demo_lin_storage_os_db_disk1"
    caching           = "ReadWrite"
...
resource "azurerm_virtual_machine_extension" "demo_db_build" {
  name                 = "demo_lin_db_build"
  location             = "${azurerm_resource_group.demo_resource_group.location}"
  resource_group_name  = "${azurerm_resource_group.demo_resource_group.name}"
  virtual_machine_name = "${azurerm_virtual_machine.demo_db01.name}"
